...
export default function QuizList({ quizList }) {
    const { fetchQuizList, setQuizPage, showLoadingComponent, ascending } =
        useGlobalContext();
    ...
    if (showLoadingComponent) return <LoadingQuizList />;
            ...
            {quizList.length > 0 ? (
                quizList.map((quiz, index) => {
                    ...
                    return (
                        <div
                            key={index}
                            className="container drop-shadow-xl shadow-xl dark:bg-blue-950 dark:text-white bg-blue-100 px-2 py-2 rounded-xl hover:-translate-y-2 transition-all duration-500  overflow-hidden max-w-[300px] mx-auto aspect-[1.6/1]"
                        >
                ...
                <div className="mt-4">
                    ...
                    <button
                        className="bg-orange-200 dark:bg-orange-800 dark:text-white rounded-md px-2 py-1 drop-shadow-md shadow-md hover:scale-105 active:scale-95 transition-all duration-100"
